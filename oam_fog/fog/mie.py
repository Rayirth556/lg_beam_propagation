...
class MieModel:
    ...
    def summary(self, fog_config):
        ...
        print(f"Mie scattering summary  (r_eff={self.r_eff*1e6:.1f} um, "
              f"lambda={self.wavelength*1e9:.0f} nm, x={self.x:.3f})")
        print(f"  Q_ext={self.Q_ext:.4f}  Q_sca={self.Q_sca:.4f}  g={self.g:.4f}")
        print(f"  s_ext={self.sigma_ext:.3e} m2   s_sca={self.sigma_sca:.3e} m2")
        print(f"{'='*65}")
        print(f"{'N_d [m^-3]':>14}  {'vis [m]':>8}  {'T_amp/screen':>13}  {'s_A':>8}  {'s_phi [rad]':>10}")
        print(f"{'-'*65}")
        for nd in nd_values:
